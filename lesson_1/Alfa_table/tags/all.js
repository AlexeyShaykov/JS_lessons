riot.tag('chessboard', '<table width="100%" border="1" cellpadding="4" cellspacing="0" class="first"> <tr each="{rows}"> <td each="{columns}"></td> </tr> </table>', function(opts) {
...
riot.tag('table-tds', '<table width="100%" border="1" cellpadding="4" cellspacing="0" class="first"> <tr each="{rows}"> <td each="{columns}" onclick="{ changeColor }"></td> </tr> </table>', function(opts) {
	this.on('mount', function(){
		columns = new Array(opts.collum);
		rows = new Array(opts.row);
		this.update();
